        ...
        for rows in excelList:
            for i in range(0, maxColumnCnt):
                ...
                if i==0:
                    ...
                    findImgWithText(gui.locateOnScreen(imgDirPath + '사업타이틀.png'), data)
                    continue
                elif i==11:
...
# 현재 데이터를 포함하는 이미지의 텍스트 찾아서 클릭 #
def findImgWithText(imgPath, data):
    window_width, window_height = gui.size()




    # # 이미지에서 텍스트 추출
    # extractedTxt = pytesseract.image_to_string(imgPath)

    # # '일반사업'이 포함되는지 확인
    # if data in extractedTxt:
    #     # 해당 이미지 오른쪽 select 클릭
    #     imgRightClick(imgPath)

    # else:
    #     gui.alert('찾고자 하는 이미지가 존재하지 않습니다. \n관리자 확인이 필요합니다.')
    #     return False
# def findImgWithText End #
