    ...
    try:
        ...
        for row_i in range(0, len(excel_list)) :
            ...
            for i in range(0, max_col_cnt):
                ...
                if i==0:
                    find_and_click.xy_info_click(xy_info['income_expense_type'])
                    find_and_click.xy_info_click(xy_info['income']) if data == '수입' else find_and_click.xy_info_click(xy_info['expense'])
                    continue
                elif i==2:
                    find_and_click.xy_info_click(xy_info['project_combobox'])
                    find_and_click.customization_project_img_click(self)
                    continue
                elif i==3:
                    # 결의일자 활성화
                    find_and_click.xy_info_click(xy_info['cashier_dt'])
                    # 결의일자 삽입
                    gui.hotkey('ctrl', 'a')
                    gui.press('backspace')
                    pyperclip.copy(data)
                    ...
                    find_and_click.xy_info_click(xy_info['account_subject_cd_box'])
                    gui.press('tab')
                    pyperclip.copy(data)
                    gui.hotkey('ctrl', 'v')
                    gui.press('tab')
                    ...
                    if data == '본인부담금수입':
                        find_and_click.xy_info_click(xy_info['subject_box'])
                        time.sleep(0.5)
                        gui.press('enter')
                    # 장기요양급여수입인 경우 팝업창이 오픈되는데 반영을 기준으로 선택하도록 한다.
                    elif data == '장기요양급여수입':
                        find_and_click.pick_account_반영(data, 'account_subject', self)
                    continue
                elif i==5:
                    find_and_click.xy_info_click(xy_info['summary']) #find_and_click.img_click('결의서적요.png')
                    gui.hotkey('ctrl', 'a')
                    gui.press('backspace')
                    pyperclip.copy(data)
                    gui.hotkey('ctrl', 'v')
                    ...
                    find_and_click.xy_info_click(xy_info['amount']) #find_and_click.img_right_click('금액타이틀.png')
                    ...
                    if data != '':
                        find_and_click.xy_info_click(xy_info['opponent_account_subject_cd_box']) #상대계정박스 #find_and_click.img_click('상대계정박스.png')
                        gui.press('tab')
                        pyperclip.copy(data)
                        gui.hotkey('ctrl', 'v')
                        # time.sleep(0.2)
                        ...
                        find_and_click.pick_account_반영(data, 'opponent_subject', self) if data == '장기요양급여수입' else None
                        continue
                    else:
                        continue
            ...
            if i == max_col_cnt-1:
                # time.sleep(0.5)
                # find_and_click.xy_info_click(xy_info['search']) # 조회 #find_and_click.img_left_click('조회.png') # 포커스 초기화 클릭
                # time.sleep(0.2)
