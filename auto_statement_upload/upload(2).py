...
########## Setting PyQt5 Design #######################################################################
app = QApplication(sys.argv)
window = QWidget()
window.show() # PyQt5 오픈
...
'''
    PyQT5 설정
    __init__(self) : 윈도우의 기본 세팅
'''
class window_setting() :
    def __init__(self) :
        super().__init__()
        self.setWindowTitle('전표등록 자동업로드 작업 프로그램')
...
# ########## Init UI Form ##############################################################################
# # 루트화면에 보여질 정적 요소들
# upload_file_nm = Entry(window) # 업로드파일명 입력창
...
# Label(window, text='첨부파일명').place(x=5, y=100)
# # 업로드파일명 입력창
# upload_file_nm = Entry(window).place(x=10, y=100)
